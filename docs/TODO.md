...
- [x] **`database.py` - Dual DB Sytem:**
  ...
  - [x] Add a method to promote a sender's classification from the suggestion DB to the validated DB.
...
- [x] **`app.py` - Update Validation Workflow:**
  - [x] Change the `--validate` flag's behavior to promote a classification to the validated DB.
- [x] **`tests/` - Update Tests:**
  - [x] Update all relevant tests to reflect the new dual-database system and the updated AMSC logic.
...
- [x] **`config.py` & `config.toml` - Add Fast Parse Settings:**
  - [x] Add `fast_parse_batch_size` to control the number of emails fetched in Pass 1.
  - [x] Add `folder_cache_ttl_hours` for the IMAP folder hierarchy cache.
  - [x] Add `unclassified_folder_name` for AI fallback.
- [x] **`imap_service.py` - Implement Fast Parse Methods:**
  - [x] Implement `get_folder_hierarchy()` to fetch and cache the folder list in `data/imap_folders.json`.
  - [x] Implement `get_email_senders(uids: list[str])` to fetch only the `From` header for a batch of emails.
  - [x] Implement `get_full_emails(uids: list[str])` for Pass 2 processing.
  - [x] Deprecate the existing `get_emails()` method.
- [x] **`app.py` - Implement Two-Pass Orchestration:**
  - [x] Rework the main application logic to manage the two-pass system.
  - [x] Implement Pass 1: Batch fetch UIDs, get senders, check against the database, and move matches.
  - [x] Implement Pass 2: Process remaining UIDs with the full AI classification logic.
  - [x] Implement the fallback strategy to move unclassified emails to the folder specified in `unclassified_folder_name`.
